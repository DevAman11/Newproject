...
function Update() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = fetch(`http://localhost:9000/Update/${Id}`, {
        method: "PATCH",
        headers: {
      ...
      if (!response.ok) {
        ...
        alert(`Update Data Successfully : ${JSON.stringify(data)}`);
      }
    } catch (error) {
      console.log(error);
  ...
  const UserData = async () => {
    try {
      const res = await fetch(`http://localhost:9000/Update/${Id}`)
